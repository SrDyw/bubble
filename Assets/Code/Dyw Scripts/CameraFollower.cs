...
public class CameraFollower : MonoBehaviour
{
    ...
    [SerializeField] private bool useUnscaleTime;
    ...
    public float DeltaTime { get => useUnscaleTime ? Time.unscaledDeltaTime : Time.deltaTime; }
    ...
    void Update()
    {
        if (!inCinematic)
        {
            ...
            else if (followTo != null)
            {
                ...
                transform.position = interpolate ? Vector3.Lerp(transform.position, clampedPosition, 0.2f) : clampedPosition;
    ...
    Vector3 ClampPosition(Vector3 positionTarget)
    {
        ...
        if (clampToCollider)
        {
            clampedPosition = new Vector3(
                ...
                Mathf.Clamp(positionTarget.y, clampToCollider.bounds.min.y + _camera.orthographicSize, clampToCollider.bounds.max.y - _camera.orthographicSize),
                positionTarget.z
            );
        }
        ...
        if (colliderSizeY <= _camera.orthographicSize * 2) clampedPosition.x = clampToCollider.bounds.min.y + colliderSizeY;
    ...
    Vector3 GeneratePositionOffset(Vector3 position)
    {
        ...
        return Vector3.Lerp(transform.position, position + vecOffset, DeltaTime * speedMovement);
    }
    ...
    IEnumerator MoveBtwPoints()
    {
        ...
        {
            Vector3 positionTarget = Vector2.Lerp(transform.position, cinematicPoints[currentCP], DeltaTime * cinematicSpeed);
